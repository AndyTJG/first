...
if(!preg_match("/^[a-z A-Z]*$/", $nama)){
 ...
 if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/gif" || $jenis_gambar=="image/png")
    ...
    $cek = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM pengguna WHERE nama='$nama' or username='$username'"));

    if ($cek > 0){
    echo "<script>window.alert('nama admin username  sudah ada')
    window.location='input_user.php'</script>";
    }else {
    mysqli_query($koneksi, "INSERT INTO pengguna (id_user, nama, alamat, username, password, level, gambar ) VALUES
                  ('$id_user', '$nama', '$alamat', '$username', '$password', '$level', '$gambar')");
